...
- The monitor intentionally scans the 256-byte memory page, so the sub-64-byte program image is visible as black-and-white noise pixels in the top-left portion of the screen. Each memory bit is drawn as a taller light-gray block on black, approximating the stacked pixels from the ELF-II video output.
